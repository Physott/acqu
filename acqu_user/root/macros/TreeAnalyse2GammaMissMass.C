...
void	TreeAnalyse2GammaMissMass::Analyse(const Char_t* FileName)
{
	TreeAnalyse2GammaMissMass* c = new TreeAnalyse2GammaMissMass(FileName);
		c->Open();
		if(!c->IsOpen())
			return;
		c->Analyse();
...
void	TreeAnalyse2GammaMissMass::AnalyseFolder(const Char_t* FolderName)
{
	cout << "***********************************************************************" << endl;
	cout << "********************TreeAnalyse2GammaMissMass**************************" << endl;
	cout << "***********************************************************************" << endl;
	...
	if (files) 
	{
		...
		while(sfile=(TSystemFile*)next())
		{
			...
			{
				if(fname.EndsWith("2g_IMPi0.root") || fname.EndsWith("2g_IMEta.root") || fname.EndsWith("2g_IMEtaP.root") || fname.EndsWith("2gProton_IMPi0.root") || fname.EndsWith("2gProton_IMEta.root") || fname.EndsWith("2gProton_IMEtaP.root"))
					TreeAnalyse2GammaMissMass::Analyse(fname.Data());
				else
					printf("\n");
			}
			else
