...
private:
	...
	bool				isOpened;
	Cut*				cutCBTime;
	ReadRootTreeCanvas*	canvasGeneral;
	ReadRootTreeCanvas*	canvasCut;
	int					period;
	int					nPeriod;
...
protected:
	...
	virtual	void	Fill(ReadRootTreeCanvas* can);
...
public:
	...
	virtual	void	Clear()											{canvasGeneral->Clear(); cutCBTime->Clear(); canvasCut->Clear();}
			
	virtual	void	Analyse(const int min, const int max);
	
	virtual	void	Draw()											{canvasGeneral->Draw(); cutCBTime->Draw(); canvasCut->Draw();}
